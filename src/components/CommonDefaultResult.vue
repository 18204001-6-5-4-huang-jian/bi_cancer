...
<template>
  <div>
    <div class="content">
      ...
      :th1="'省份'"
      :th2="contentTitle2.substr(0,contentTitle2.length-2) + '数'"
      ></pop-table>
    </div>
  </div>
...
export default {
  ...
  watch: {
    rightData:function(val){
      if(val){
       this.total = val.dataList[0].num;
       this.percent = (val.dataList[0].rate*100).toFixed(2)
       let barChart = this.$echarts.init(this.$refs["barChart"]);
       this.barChart = barChart;
       this.barSeriesData = val.topList.barSeriesData;
       this.yAxisData = val.topList.yAxisData;
       this.drawBar();
       this.init("barChart");
      }
      
    },
    mapChartData:function(val){
  ...
  mounted() {
    if(this.rightData){
       this.total = this.rightData.dataList[0].num;
       this.percent = (this.rightData.dataList[0].rate*100).toFixed(2)
    }
  },
  components: {
    PopTable
  ...
  methods: {
    ...
    // 加载地图数据
     getMapData() {
        this.mapDataNew = this.mapChartData;
        this.mapChartData.rows.filter((item) => {
          item.name = item.ROVINCE;
          item.value = item.SUMDATA;
        })
        this.mapData=this.mapChartData.rows;
        this.tableData = this.mapChartData.rows;
        // map图例的最大值、最小值
        let arr = []
        this.mapChartData.rows.filter(item =>{
    ...
    drawBar() {
      let self = this;
      // console.log(self.yAxisData);
      // console.log(self.barSeriesData);
      //初始化数据
      let option = {
        tooltip: {
          trigger: "axis",
        ...
        series: [
          {
            type: "bar",
            data: (self.barSeriesData).reverse(),
            barWidth: 26,
            barCategoryGap: "50%",
            smooth: true,
            cursor: "default",

...
export default {
  ...
  methods: {
    drawPie() {
      ...
      let objFirseList = this.objectMerge(this.dataList, this.dimension1);
      objFirseList.filter((item, index) => {
      ...
      if (this.dimension2) {
        ...
        objSecondList = this.objectMerge(this.dataList, this.dimension2);
        objSecondList.filter((item, index) => {
          item.value = this.sum(this.arrayMerge(item.data, this.indexarr));
          item.name = item[this.dimension2];
        });
        let seriesItem = {
          // name: "数量",
          type: "pie",
